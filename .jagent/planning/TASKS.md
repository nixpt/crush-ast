...
- [ ] **gap** — CRUSH-65: JS Math.sin/Math.cos/Math.tan are absent from lower_swc.rs's Math.* match arm entirely, so they fall to the default passthrough and silently miscompile via compiler.rs's dotted method-call path (load Math + cap_call sin) -- the exact same wrong-answer-no-error symptom as the Math.floor case-mismatch. The consumers DO exist: MathSinCap/MathCosCap/MathTanCap are registered as math.sin/math.cos/math.tan in crush-lang-sdk/src/stdlib.rs:344-346. Fix is to add them to the CapabilityCall arm alongside min/max. Not done in CRUSH-65 because those names were never in the producer list (missing feature, not the case-mismatch defect in scope).  _(panini-crush, 2026-07-24)_
