...
- [ ] **issue** — Compiled FastVM array-loop repro also exposes a separate optimizer/codegen defect: `total = total + item` lowers to `LoadLocal(item); StoreLocal(total)`, dropping the addition. Keep this out of the array mutation fix or file a follow-up before relying on accumulator arithmetic in optimized tests.  _(unknown, 2026-08-22)_
