...
pub fn run_zig(source: &str) -> anyhow::Result<String> {
    ...
    let (_, cast) = adapter.walk(source, "test.zig")
        .map_err(|e| anyhow::anyhow!("zig->CAST: {e}"))?;
    ...
    for name in &["__crush_assign__", "append", "push"] {
...
mod tests {
    ...
    #[test]
    fn test_zig_walk_no_crash() {
        let src = "pub fn main() void { var x: i32 = 42; }";
        let r = run_zig(src);
        assert!(r.is_ok(), "zig walk should not crash: {r:?}");
    }

    #[test]
    fn test_zig_adapter_in_registry() {
        let mut registry = walker_core::AdapterRegistry::new();
        registry.register(Box::new(crate::ZigAdapter));
        let result = registry.walk("pub fn main() void {}", "test.zig");
        assert!(result.is_ok(), "zig adapter should walk via registry: {result:?}");
    }
}
