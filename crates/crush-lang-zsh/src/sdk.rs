...
use crate::ZshFrontend;
use walker_core::frontend_pipeline;

pub fn run_zsh(source: &str) -> anyhow::Result<String> {
    let frontend = ZshFrontend;
    let (_, cast) = frontend_pipeline(&frontend, source)
        .map_err(|e| anyhow::anyhow!("zsh->CAST: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn test_zsh_walk_no_crash() {
        let src = "x=42; echo $x";
        let r = run_zsh(src);
        assert!(r.is_ok(), "zsh walk should not crash: {r:?}");
    }
}
