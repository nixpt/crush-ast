...
impl<'a> Visitor<'a> {
    fn visit_statement(&mut self, node: Node) -> Result<Option<Statement>> {
        let meta = self.base.create_meta(node, "rust", self.file_name);
        match node.kind() {
            "function_item" => {
                ...
                for p_node in params_node.children(&mut params_node.walk()) {
                    if p_node.kind() == "parameter" {
                        let pattern = p_node.child_by_field_name("pattern").unwrap();
                        params.push((self.base.text(pattern)?.to_string(), CastType::Any));
                    }
                }
            ...
            "let_declaration" => {
                ...
                let name = self.base.text(pattern)?.to_string();
    ...
    fn visit_block(&mut self, node: Node) -> Result<Vec<Statement>> {
        ...
        // For Rust, blocks often return the last expression without a semicolon
        for child in node.children(&mut node.walk()) {
            ...
            {
                // Last expression in a block (implied return)
                if child
                    .next_sibling()
                    .map(|n| n.kind() == "}")
                    .unwrap_or(true)
                {
                    ...
                    body.push(Statement::Return {
                        value: Some(expr),
                        meta: self.base.create_meta(child, "rust", self.file_name),
                    });
                } else {
                    let expr = self.visit_expression(child)?;
    ...
    fn visit_expression(&mut self, node: Node) -> Result<Expression> {
        ...
        match node.kind() {
            ...
            "integer_literal" => Ok(Expression::IntLiteral {
            ...
            "unit_expression" => Ok(Expression::NullLiteral { meta }),
            "binary_expression" => {
                let left =
                    Box::new(self.visit_expression(node.child_by_field_name("left").unwrap())?);
                let operator = self.base.text(node.child(1).unwrap())?.to_string(); // middle child is usually op
                let right =
            ...
            "call_expression" => {
                ...
                if let Some(cap_name) = walker_core::map_to_capability("rust", func_name) {
