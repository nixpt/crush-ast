...
conv_cap!(ConvToBoolCap, "to_bool", 1, |args: &[Value]| {
    ...
    let result = match v {
        ...
        Value::Tuple(t) => Value::Int(if t.is_empty() { 0 } else { 1 }),
        Value::List(l) => Value::Int(if l.borrow().is_empty() { 0 } else { 1 }),
        Value::Vector(v) => Value::Int(if v.borrow().is_empty() { 0 } else { 1 }),
        Value::Set(s) => Value::Int(if s.borrow().is_empty() { 0 } else { 1 }),
