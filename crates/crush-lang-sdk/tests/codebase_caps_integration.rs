...
fn integration_annotation_history_cap_via_runtime() {
    // CRUSH-31 review followup: refactored off the manual
    // `codebase::register_at` cap-build pattern. The chain
    // `with_codebase_at` → `with_dejavue` is the canonical host
    // integration — the manual patch lives only in
    // `codebase_stale_e2e.rs` (older test, kept for pre-builder
    // registry-shape coverage).
    //
    // The shape covered stays identical to the pre-followup version:
    // timeline with TWO decision events for `use-workspace-deps`
...
"#;
    ...
    let rt = Runtime::new()
        .with_codebase_at(&[("hist_fixture", src)], pin_today())
