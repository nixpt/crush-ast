...
use crush_cast::{Statement, Expression, CastType};
use syn::Stmt;
...
pub fn lower_stmt(stmt: &Stmt) -> anyhow::Result<Statement> {
    ...
    match stmt {
        Stmt::Local(local) => {
            ...
            Ok(Statement::VarDecl { name, value, type_hint: CastType::Any, meta })
        }
        Stmt::Item(item) => {
            match item {
                syn::Item::Fn(item_fn) => {
                    let name = item_fn.sig.ident.to_string();
                    let params: Vec<(String, CastType)> = item_fn.sig.inputs.iter().map(|p| {
                        match p {
                    ...
                    Ok(Statement::FunctionDef { name, params, body, meta })
                }
        ...
        Stmt::Macro(mac) => {
            let mac_name = mac.mac.path.get_ident()
                .map(|i| i.to_string())
                .unwrap_or_default();
