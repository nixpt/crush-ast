...
fn test_analyze(source: &str, ext: &str) -> crush_walker_core::FeatureReport {
...
fn test_js_frontend_arrow_function() {
    ...
    let frontend = JsFrontend::new("js");
    let (_report, program) = crush_walker_core::frontend_pipeline(&frontend, source).unwrap();
    assert!(program.functions.contains_key("main"));
}
...
fn test_js_frontend_import() {
    ...
    let frontend = JsFrontend::new("js");
    let (report, _program) = crush_walker_core::frontend_pipeline(&frontend, source).unwrap();
    assert!(!report.can_lower_safely());
    assert!(report.dangerous_imports.contains(&"fs".to_string()));
}
