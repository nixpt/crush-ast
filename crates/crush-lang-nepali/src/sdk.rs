...
pub fn run_nepcode(source: &str) -> anyhow::Result<String> {
    ...
    let (_, cast) = adapter.walk(source, "test.np").map_err(|e| anyhow::anyhow!("nepcode->CAST: {e}"))?;
...
mod tests {
    ...
    #[test] fn test_nepcode_compile() { assert!(run_nepcode("x = 42; print(x)").is_ok()); }
    #[test] fn test_nepcode_registry() {
        let mut r = AdapterRegistry::new();
        r.register(Box::new(crate::NepcodeAdapter));
        assert!(r.walk("x = 1", "test.np").is_ok());
    }
}
