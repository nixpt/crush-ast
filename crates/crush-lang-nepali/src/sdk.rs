//! Nepcode SDK — full pipeline from Nepali/Devnagari source to CVM1 execution.
#![cfg(test)]

pub fn run_nepcode(source: &str) -> anyhow::Result<String> {
    let cast = crate::nepali_to_cast(source)
        .map_err(|e| anyhow::anyhow!("nepcode->CAST: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn test_nepcode_walk_no_crash() {
        let src = "x = 42; print(x)";
        let r = run_nepcode(src);
        assert!(r.is_ok(), "nepcode walk should not crash: {r:?}");
    }
}
